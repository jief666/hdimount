...
int test_HMAC_SHA256_openssl(const uint8_t *key, int key_len, const unsigned char *d, size_t n,
...
int test_HMAC_SHA256_crypto(const uint8_t *key, int key_len, const unsigned char *d, size_t n,
							      const unsigned char *expected_result, int expected_result_len)
{
	int ret = 0;
	unsigned char out[expected_result_len];
...
int test_HMAC_SHA256()
{
	...
	int ret;
	ret = test_HMAC_SHA256_openssl(key, sizeof(key), data, sizeof(data), expected_result, sizeof(expected_result));
	fprintf(stderr, "test_HMAC_SHA256_openssl %d\n", ret);
	if ( ret == 0 ) all_tests_ok = false;

	ret = test_HMAC_SHA256_crypto(key, sizeof(key), data, sizeof(data), expected_result, sizeof(expected_result));
	fprintf(stderr, "test_HMAC_SHA256_crypto %d\n", ret);
	if ( ret == 0 ) all_tests_ok = false;
