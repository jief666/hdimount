...
int test_HMAC_SHA256_crypto_openssl(const uint8_t *key, int key_len, const unsigned char *d, size_t n,
							      const unsigned char *expected_result, int expected_result_len)
{
	...
	unsigned char* rv = HMAC(EVP_sha256(), key, key_len, d, n, out, &outl);

	if ( rv == NULL )
		goto err;
	if ( outl > INT_MAX )
		goto err;
	if ( (int)outl != expected_result_len )
		goto err;
